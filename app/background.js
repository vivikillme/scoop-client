(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		var a = typeof exports === 'object' ? factory(require("better-sqlite3")) : factory(root["better-sqlite3"]);
		for(var i in a) (typeof exports === 'object' ? exports : root)[i] = a[i];
	}
})(global, (__WEBPACK_EXTERNAL_MODULE_better_sqlite3__) => {
...
/* harmony export */   cacheSearchResults: () => (/* binding */ cacheSearchResults),
/* harmony export */   closeDatabase: () => (/* binding */ closeDatabase),
/* harmony export */   getCachedBuckets: () => (/* binding */ getCachedBuckets),
/* harmony export */   getCachedSearchResults: () => (/* binding */ getCachedSearchResults),
/* harmony export */   getDbPath: () => (/* binding */ getDbPath),
/* harmony export */   getInstalledApps: () => (/* binding */ getInstalledApps),
/* harmony export */   initDatabase: () => (/* binding */ initDatabase),
/* harmony export */   saveBuckets: () => (/* binding */ saveBuckets),
/* harmony export */   saveInstalledApps: () => (/* binding */ saveInstalledApps)
/* harmony export */ });
...
let db = null;
function getDbPath() {
  const userDataPath = electron__WEBPACK_IMPORTED_MODULE_4__.app.getPath('userData');
  return path__WEBPACK_IMPORTED_MODULE_3__.join(userDataPath, 'scoop-client.db');
}
function initDatabase() {
  try {
    const dbPath = getDbPath();
    db = new (better_sqlite3__WEBPACK_IMPORTED_MODULE_2___default())(dbPath);
...
function saveInstalledApps(apps) {
  ...
  try {
    ...
    `);
    const appNames = _babel_runtime_corejs3_core_js_stable_instance_map__WEBPACK_IMPORTED_MODULE_0___default()(apps).call(apps, a => a.name);
    const deletePlaceholder = appNames.length > 0 ? _babel_runtime_corejs3_core_js_stable_instance_map__WEBPACK_IMPORTED_MODULE_0___default()(appNames).call(appNames, () => '?').join(',') : 'NULL';
...
function getInstalledApps() {
  if (!db) return [];
  try {
    const rows = db.prepare('SELECT name, version, bucket FROM installed_apps').all();
    return _babel_runtime_corejs3_core_js_stable_instance_map__WEBPACK_IMPORTED_MODULE_0___default()(rows).call(rows, row => ({
      name: row.name,
      version: row.version,
      bucket: row.bucket
    }));
...
function cacheSearchResults(query, results) {
  ...
  try {
    ...
    `);
    insert.run(query, _babel_runtime_corejs3_core_js_stable_json_stringify__WEBPACK_IMPORTED_MODULE_1___default()(results));
  } catch (error) {
...
function saveBuckets(buckets) {
  ...
  try {
    ...
    `);
    const bucketNames = _babel_runtime_corejs3_core_js_stable_instance_map__WEBPACK_IMPORTED_MODULE_0___default()(buckets).call(buckets, b => b.name);
    const deletePlaceholder = bucketNames.length > 0 ? _babel_runtime_corejs3_core_js_stable_instance_map__WEBPACK_IMPORTED_MODULE_0___default()(bucketNames).call(bucketNames, () => '?').join(',') : 'NULL';
...
function getCachedBuckets() {
  if (!db) return [];
  try {
    const rows = db.prepare('SELECT name, source FROM buckets').all();
    return _babel_runtime_corejs3_core_js_stable_instance_map__WEBPACK_IMPORTED_MODULE_0___default()(rows).call(rows, row => ({
      name: row.name,
      source: row.source
    }));
...
/* harmony import */ var _babel_runtime_corejs3_core_js_stable_instance_sort__WEBPACK_IMPORTED_MODULE_10__ = __webpack_require__(/*! @babel/runtime-corejs3/core-js-stable/instance/sort */ "./node_modules/@babel/runtime-corejs3/core-js-stable/instance/sort.js");
...
/* harmony import */ var _database__WEBPACK_IMPORTED_MODULE_17__ = __webpack_require__(/*! ./database */ "./main/database.ts");
...
const execAsync = (0,util__WEBPACK_IMPORTED_MODULE_15__.promisify)(child_process__WEBPACK_IMPORTED_MODULE_14__.exec);
let mainWindow = null;

// Check if Scoop is installed
async function checkScoopInstalled() {
  try {
...
async function executeScoop(args) {
  return new (_babel_runtime_corejs3_core_js_stable_promise__WEBPACK_IMPORTED_MODULE_9___default())(resolve => {
    const proc = (0,child_process__WEBPACK_IMPORTED_MODULE_14__.spawn)('scoop', args, {
      shell: true,
...
function createWindow() {
  mainWindow = new electron__WEBPACK_IMPORTED_MODULE_12__.BrowserWindow({
    width: 1200,
    height: 800,
    minWidth: 900,
    minHeight: 600,
    webPreferences: {
      ...
      preload: path__WEBPACK_IMPORTED_MODULE_13__.join(__dirname, 'preload.js')
    },
    titleBarStyle: 'hiddenInset',
    // frame: process.platform === 'darwin' ? true : false,
    frame: true,
    title: 'ScoopClient',
    icon: path__WEBPACK_IMPORTED_MODULE_13__.join(__dirname, '../resources/icon.png')
  });

  // Development mode - Nextron passes the port as an argument
  const isDev = !electron__WEBPACK_IMPORTED_MODULE_12__.app.isPackaged;
  if (isDev) {
    ...
    mainWindow.loadFile(path__WEBPACK_IMPORTED_MODULE_13__.join(__dirname, '../renderer/out/index.html'));
  }
...
electron__WEBPACK_IMPORTED_MODULE_12__.app.whenReady().then(async () => {
  // Initialize database (optional)
  try {
    (0,_database__WEBPACK_IMPORTED_MODULE_17__.initDatabase)();
  } catch (error) {
    console.log('Database initialization skipped:', error);
  }
  createWindow();
  electron__WEBPACK_IMPORTED_MODULE_12__.app.on('activate', () => {
    if (electron__WEBPACK_IMPORTED_MODULE_12__.BrowserWindow.getAllWindows().length === 0) {
      createWindow();
    }
  });
});
electron__WEBPACK_IMPORTED_MODULE_12__.app.on('window-all-closed', () => {
  (0,_database__WEBPACK_IMPORTED_MODULE_17__.closeDatabase)();
...
// IPC Handlers
electron__WEBPACK_IMPORTED_MODULE_12__.ipcMain.handle('check-scoop', async () => {
  return checkScoopInstalled();
});
electron__WEBPACK_IMPORTED_MODULE_12__.ipcMain.handle('scoop-search', async (_, query) => {
  return executeScoop(['search', query]);
});
electron__WEBPACK_IMPORTED_MODULE_12__.ipcMain.handle('scoop-install', async (_, app) => {
  return executeScoop(['install', app]);
});
electron__WEBPACK_IMPORTED_MODULE_12__.ipcMain.handle('scoop-uninstall', async (_, app) => {
  return executeScoop(['uninstall', app]);
});
electron__WEBPACK_IMPORTED_MODULE_12__.ipcMain.handle('scoop-list', async () => {
  return executeScoop(['list']);
});
electron__WEBPACK_IMPORTED_MODULE_12__.ipcMain.handle('scoop-update', async (_, app) => {
  return executeScoop(['update', app || '*']);
});
electron__WEBPACK_IMPORTED_MODULE_12__.ipcMain.handle('scoop-info', async (_, app) => {
  return executeScoop(['info', app]);
});
electron__WEBPACK_IMPORTED_MODULE_12__.ipcMain.handle('scoop-bucket-list', async () => {
  return executeScoop(['bucket', 'list']);
});
electron__WEBPACK_IMPORTED_MODULE_12__.ipcMain.handle('scoop-bucket-add', async (_, name, repo) => {
  const args = ['bucket', 'add', name];
  if (repo) args.push(repo);
  return executeScoop(args);
});
electron__WEBPACK_IMPORTED_MODULE_12__.ipcMain.handle('scoop-bucket-rm', async (_, name) => {
  return executeScoop(['bucket', 'rm', name]);
});
electron__WEBPACK_IMPORTED_MODULE_12__.ipcMain.handle('scoop-reset', async (_, app) => {
  return executeScoop(['reset', app]);
});
electron__WEBPACK_IMPORTED_MODULE_12__.ipcMain.handle('scoop-cleanup', async (_, app) => {
  return executeScoop(['cleanup', app || '*']);
});
electron__WEBPACK_IMPORTED_MODULE_12__.ipcMain.handle('scoop-status', async () => {
  return executeScoop(['status']);
});

// Get installed versions for an app (for version switching)
electron__WEBPACK_IMPORTED_MODULE_12__.ipcMain.handle('scoop-versions', async (_, appName) => {
  try {
    ...
    const appsDir = path__WEBPACK_IMPORTED_MODULE_13__.join(homeDir, 'scoop', 'apps', appName);
    const currentLink = path__WEBPACK_IMPORTED_MODULE_13__.join(appsDir, 'current');

    // Check if app directory exists
    if (!fs__WEBPACK_IMPORTED_MODULE_16__.existsSync(appsDir)) {
      return {
        versions: [],
        currentVersion: '',
        error: 'App not found'
    ...
    let currentVersion = '';
    try {
      if (fs__WEBPACK_IMPORTED_MODULE_16__.existsSync(currentLink)) {
        const currentTarget = fs__WEBPACK_IMPORTED_MODULE_16__.readlinkSync(currentLink);
        currentVersion = path__WEBPACK_IMPORTED_MODULE_13__.basename(currentTarget);
      }
    } catch (e) {
    ...
    const entries = fs__WEBPACK_IMPORTED_MODULE_16__.readdirSync(appsDir, {
      withFileTypes: true
    });
    const versions = _babel_runtime_corejs3_core_js_stable_instance_sort__WEBPACK_IMPORTED_MODULE_10___default()(_context = _babel_runtime_corejs3_core_js_stable_instance_map__WEBPACK_IMPORTED_MODULE_11___default()(_context2 = _babel_runtime_corejs3_core_js_stable_instance_filter__WEBPACK_IMPORTED_MODULE_8___default()(entries).call(entries, entry => entry.isDirectory() && entry.name !== 'current')).call(_context2, entry => entry.name)).call(_context, (a, b) => b.localeCompare(a, undefined, {
      numeric: true
    })); // Sort descending
...
electron__WEBPACK_IMPORTED_MODULE_12__.ipcMain.handle('open-external', async (_, url) => {
  electron__WEBPACK_IMPORTED_MODULE_12__.shell.openExternal(url);
});
electron__WEBPACK_IMPORTED_MODULE_12__.ipcMain.handle('show-message', async (_, options) => {
  if (mainWindow) {
    return electron__WEBPACK_IMPORTED_MODULE_12__.dialog.showMessageBox(mainWindow, options);
  }
  return {
    response: 0
  };
